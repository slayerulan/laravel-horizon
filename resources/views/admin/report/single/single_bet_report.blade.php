          ...
          <div class="col-sm-4">
            ...
            <select class="form-control show-tick" data-live-search="true" id='player' onChange="getData();">
                  ...
                  @foreach ($players as $player)
                      <option value="{{ $player->id }}" class="player_list player_list_{{$player->agent_id}}">{{ $player->username }}</option>
                  @endforeach
            </select>
          </div>


          <div class="demo-switch col-sm-4">
            <h5 class="card-inside-title">Bet Type</h5>
              <div class="switch">
                  <label>Live Betting<input type="checkbox" name="betType" id="betType" checked onChange="getData();"><span class="lever"></span>Prematch Betting</label>
              </div>
          </div>
        </div>
    </div>
...
function getData()
{
  ...
  else
  {
    ...
    $.ajax({
            ...
            data: {'sport' : sport, 'start_date' : startDate, 'end_date' : endDate, 'status' : status, 'agent' : agent, 'player' : player, 'bet_type' : betType },
